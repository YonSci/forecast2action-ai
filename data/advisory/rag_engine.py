...
def load_action_library() -> List[Dict[str, Any]]:
    ...
    try:
        return json.loads(ACTION_LIBRARY_PATH.read_text(encoding="utf-8"))
...
def score_entry(entry: Dict[str, Any], context: Dict[str, Any]) -> float:
    ...
    if entry_hazard == hazard:
        score += 8
    elif entry_hazard == "rainfall_watch":
        score += 1
    else:
        score -= 5
    ...
    if feedback_signal in ["strong_ground_signal", "emerging_ground_signal"]:
        text = " ".join(entry.get("actions", [])) + " " + entry.get("rationale", "")
        text = text.lower()

        if "community" in text or "verify" in text or "local" in text:
            score += 1.5
...
def build_rag_advisory(
    ...
    retrieval_summary = (
        f"Retrieved {len(retrieved_guidance)} relevant knowledge item(s) for "
        f"{context.get('hazard', '').replace('_', ' ')} / {context.get('risk_level')} / "
        f"{audience.replace('_', ' ')} using hazard type, risk level, audience, "
        f"rainfall anomaly, SPI-like score and community feedback."
    )
